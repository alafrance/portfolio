...
export default function AppExperience({t, className}: { t: TranslationDictionary, className?: string}) {
  return (
    <section className={"max-w-7xl"}>
      <h1 className={cn("text-center mt-4 mb-18", className)}>{t.navbar.experience}</h1>
      {t.experiences.map((experience, index) => (
        <div className={"flex items-center max-w-6xl pb-18 relative"} key={index}>
          <div className={"absolute w-2 bg-primary z-1 top-0 bottom-0 left-21"} />
          <Image src={experience.image} alt={"Logo " + experience.title} width={100} height={100} className={"w-20 mx-12 z-10 bg-background py-6"} />
          <div>
